...
namespace WindowsFormsApp1
{
    class Triangle
    {
        ...
        public Triangle(double A,double B, double C,double H)//констректор
        {
            a = A;
            b = B;
            c = C;
        ...
        {
            get {
                if ((b + c)>a && (b < a + c) && (c < a + b)) // сумма 2 сторон должна быть больше третьей
                    return true;
                else return false;
            }
        }
        public string TriangleType // свойство позволяющее установить, существует ли треугольник с заданными сторонами
        {
            get {
                if (ExistTriangle)
                {
                    ...
                    else if ((c == b && c != a) || (c == a && b != c) || (b == c && a != b))
                    {
